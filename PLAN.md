...
- 새 프로젝트 만들기 (+): 이름 입력 아래 상태·시작일을 선택 항목으로 펼쳐서 한 번에 받는다.
  둘 다 비워도 만들어진다 — 필수가 아니라 마찰은 유지, 상세로 다시 들어가는 왕복만 줄인다
  (2026-07-17 개정). 시작일은 모던 date picker(`DatePickerModal`)로 고른다 — 상세 화면과 공용.
...
11. **데스크탑 앱 로드맵 (2026-08-14, 아이디어 단계 — 착수 아님)**: 지금 Expo web 빌드를
    Electron/Tauri로 감싸는 게 1인 프로젝트에서 현실적인 경로. 이 위에 칸반뷰·3D뷰·캔버스·
    마인드그래프 같은 **데스크탑(=웹) 전용** 화면을 얹는 걸 구상 중 — 모바일에 똑같이 이식해야
    한다는 제약 없이 `Platform.OS === 'web'` 분기로 붙인다.
    마인드그래프는 SPEC §7의 브레인맵(연결 저장) 금지에 걸리므로, **DB에 엣지를 저장하지 않고
    무리/유사도를 그때그때 재계산해서 그리는 시각화로만** 접근한다(SPEC §7 해제된 항목 참고).
