...
**제목 맨 앞에 그 항목이 나온 각도의 slot을 대괄호 라벨로 붙인다** (expansion→[확장],
idea→[아이디어], lens→[관점], resurface→[되꺼냄]). 주어진 slot을 그대로 옮겨라.
...
async function main() {
  ...
  console.log(`${parsed.length}개 → 중복 ${gate.dropped.length}개 제거 → ${angles.length}개`);
