...
async function redditFetchUrl(url){
  let u: URL;
  try {
    u = new URL(url);
  } catch {
...
function isRedditJunk(finalUrl){
  try {
...
function extractRedditSelftext(html){
  let u: URL;
  try {
    u = new URL(finalUrl);
  } catch {
  ...
  const zone = html.slice(start, commentarea > start ? commentarea );
...
async function naverPlaceFetchUrl(url){
  let u: URL;
  try {
    u = new URL(url);
  } catch {
...
function extractNaverPlaceInfo(html){
...
async function fetchLinkMeta(
  url
){ title: string | null; description: string | null; thumbnail{
  ...
  try {
    ...
    const description = isYoutubeUrl(url)
      ...
      : (extractRedditSelftext(html, res.url || url) ??
        (naverUrl ? extractNaverPlaceInfo(html) ) ??
