...
async function main() {
  ...
  const out = await callOpenAI(env.openai, MODEL, ANGLE_SYS, block, '각도');
  let parsed;
  ...
  const label = { expansion: '확장', idea: '아이디어', lens: '관점', resurface: '되꺼냄' };
  const counts = { expansion: 0, idea: 0, lens: 0, resurface: 0 };
  ...
  console.log(`구성: 확장 ${counts.expansion} · 아이디어 ${counts.idea} · 관점 ${counts.lens} · 되꺼냄 ${counts.resurface}`);
