...
```ts
// 링크 파편의 content는 URL이라 임베딩하면 무의미 → 제목·노트를 쓴다.
// merged_from 조각의 content도 합쳐 신호를 살린다.
function embedText(f: FragmentRow): string {
  const base = f.type === 'link' ? (f.link_title ?? f.content) : f.content;
  const merged = (f.merged_from ?? []).map((p: any) => p.content).join('\n');
  return [base, f.note, merged].filter(Boolean).join('\n').trim();
}
...
```
1. select id, type, content, link_title, note, merged_from
