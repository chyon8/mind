...
import { alreadyMorningToday, observationLine } from '../discovery/observation.ts';
...
async function run(): Promise<string> {
  try {
    ...
    // discovery의 '모닝 브리핑' 버튼과 상한 로직을 공유한다 (observation.ts).
    if (await alreadyMorningToday(supabase)) {
      logGate(false, '오늘 이미 보냈다', { date: kstToday() });
      return 'already_sent_today';
    }

    const prelude = await observationLine(supabase);
