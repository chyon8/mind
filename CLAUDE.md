...
- **RUDY.md** — Rudy(어시스턴트 층) 기획서. **RUDY-STATUS.md** — 지금 상태·다음 할 일·확정값·
  기각 목록·교훈. **Rudy 관련 요청("다음 뭐야", "어디까지 했어" 등)이 오면 memory를 보지 말고
  RUDY-STATUS.md부터 읽고 답한다.** 진행상황·결정은 memory에 절대 중복 저장하지 않는다.
- **RUDY-ARCHIVE.md — 열지 마라.** 지난 세션 기록(1,100줄)이고 만료된 계획이 가득하다.
  2026-07-26에 RUDY-STATUS가 1,163줄까지 자라 세션마다 컨텍스트를 먹고, **실제 DB·코드를
  확인하는 대신 문서 서사로 추론하는 일이 반복돼서** 잘라냈다. 유저가 "그때 왜 그렇게 정했나"를
...
- **RUDY-DISCOVERY.md** — 발견 브리핑(§10-7)의 판단 기준·렌즈·통한 예시. **발견/브리핑 작업
  ...
  ⚠️ **이건 런타임 입력이다 — 문서 정리 대상이 아니다.** `npm run discover`가
  `--allowedTools Read`로 돌면서 `scripts/discover-websearch/prompt.md`의 지시대로 이 파일의
  §1·§2·§3·§5·§6·§9를 **절 번호로 집어** 읽는다. 절을 지우거나 번호를 바꾸면 발견이 조용히
  나빠진다. 같은 이유로 **PLAN·SPEC·RUDY의 절 번호도 코드 주석이 직접 참조하므로**
  (`PLAN.md §3.6`, `RUDY.md §4-F4` 등) 재구성하지 말 것.
